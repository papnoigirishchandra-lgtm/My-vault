...
function Sidebar({ isOpen, onClose }: { isOpen: boolean; onClose: () => void }) {
  ...
  return (
    <>
      ...
      <aside className={cn(
        "fixed bottom-0 left-0 top-0 z-50 flex h-[100dvh] w-64 flex-col border-r border-slate-200 bg-white shadow-sm transition-transform lg:translate-x-0 lg:shadow-none",
...
function Topbar({ onMenuClick }: { onMenuClick: () => void }) {
  ...
  return (
    <header className="fixed left-0 lg:left-64 right-0 top-0 z-30 flex h-16 items-center justify-between border-b border-slate-200 bg-white/80 backdrop-blur-md px-4 lg:px-8 shadow-sm">
      <div className="flex items-center gap-4 flex-1">
        ...
        <div className="relative w-full max-w-xs lg:max-w-sm" ref={searchContainerRef}>
          <div className="group relative">
            ...
            <Input 
              placeholder="Search..." 
              className="h-9 rounded-xl border-slate-200 bg-slate-50/50 pl-9 text-sm focus-visible:bg-white focus-visible:ring-1 focus-visible:ring-indigo-300"
              value={searchQuery}
      ...
      <div className="flex items-center gap-2 lg:gap-4 ml-4">
        {isAdmin && (
          <DropdownMenu>
            <DropdownMenuTrigger asChild>
              <Button 
                className="h-9 w-9 lg:w-auto rounded-xl gradient-button p-0 lg:px-4 text-xs font-bold text-white transition-all active:scale-95 shadow-md shadow-indigo-500/20"
              >
                <Plus className="lg:mr-1.5 h-4 w-4" />
        ...
        {!user && (
          <Button 
            ...
            variant="outline"
            className="h-9 rounded-xl px-4 text-xs font-bold text-slate-600 border-slate-200 hover:bg-slate-50 transition-all"
          >
            Login
          </Button>
        )}
